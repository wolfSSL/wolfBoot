...
static int image_sha384(struct wolfBoot_image *img, uint8_t *hash)
{
...
#else
    {
        ...
        do {
            ...
            /* PolarFire SoC DDR build: route SHA384 reads through the
             * non-cached DDR SEG window (0xC0000000 base) so cache fills
             * don't evict L2 Scratch lines (where wolfBoot's own code/stack
             * lives).  PDMA already L2-flushed the cached writes when
             * disk-load happened, so the non-cached side reads the correct
             * DDR contents.  0x82xxxxxx -> 0xC2xxxxxx.  (Only meaningful for
             * the DDR config, where images live in the cached 0x8xxxxxxx
             * window; inert for the L2-Scratch QSPI M-mode build.) */

...
/* use the linker variable, except for the sim */
#ifndef ARCH_SIM
extern unsigned int _start_wolfboot;
extern unsigned int _end_wolfboot;
uint8_t* startWolfboot = (uint8_t*)&_start_wolfboot;
uint8_t* endWolfboot = (uint8_t*)&_end_wolfboot;
#else
#error "feff"
extern uint8_t* startWolfboot;
extern uint8_t* endWolfboot;
#endif
static int wolfBoot_unseal_pubkey(WOLFTPM2_KEY* tpmKey);
#endif
...
#ifdef WOLFTPM_KEYSTORE
    ret = wolfBoot_unseal_pubkey(&tpmKey);
#else
...
static int self_sha256(uint8_t* hash)
{
    ...
    /* set the pointer and lenth */
    selfPointer = startWolfboot;
    selfLen = endWolfboot - selfPointer;

    /* TODO does this need to be done in blocks since we're reading memory? */
    while (offset < selfLen) {
        blkSz = WOLFBOOT_SHA_BLOCK_SIZE;
        if (selfLen - offset < blkSz)
            blkSz = selfLen - offset;

        /* update the hash */
#if defined(WOLFBOOT_TPM) && defined(WOLFBOOT_HASH_TPM)
        wolfTPM2_HashUpdate(&wolftpm_dev, tpmHash, selfPointer + offset, blkSz);
#else
        wc_Sha256Update(sha, selfPointer + offset, blkSz);
#endif
...
int wolfBoot_unseal_at(uint32_t nvIndex, uint8_t* out, uint32_t* outSz)
{
    ...
    ret = wolfTPM2_ExtendPCR(&wolftpm_dev, wolftpmPcrArray[0], TPM_ALG_SHA256,
        selfDigest, WOLFBOOT_SHA_DIGEST_SIZE);
    if (ret != TPM_RC_SUCCESS)
        return -ret;
...
{
    return wolfBoot_unseal_at(WOLFTPM_ENCRYPT_KEYSTORE_INDEX, key, keySz);
}
...
static int wolfBoot_unseal_pubkey(WOLFTPM2_KEY* tpmKey)
{
    ...
    wolfBoot_unseal_at(WOLFTPM_KEYSTORE_INDEX, tpmPubkey, &tpmPubkeySz);
...
#if !defined(WOLFBOOT_RENESAS_SCEPROTECT) && !defined(WOLFTPM_KEYSTORE)
        key_slot = keyslot_id_by_sha(pubkey_hint);
        if (key_slot < 0) {
            return -1; /* Key was not found */
        }
#else
        /* SCE wrapped key is installed at RENESAS_SCE_INSTALLEDKEY_ADDR and
         * the tpm keystore will get it from the tpm */
        key_slot = 0;
#endif
    } else {

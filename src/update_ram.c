...
backup_on_failure:
    ...
    if (image_ptr) {
        if (*((uint32_t*)image_ptr) == UBOOT_IMG_HDR_MAGIC) {
            ...
            /* Honor the uImage's load address: U-Boot bootm copies the
             * payload to ih_load and jumps to ih_ep, because PowerPC /
             * VxWorks kernels are typically built non-relocatable with
             * absolute references baked in. Falling back to the wolfBoot
             * default WOLFBOOT_LOAD_ADDRESS would place the kernel at the
             * wrong address and the first internal jump would fault.
             *
             * Note: this override applies only when ih_load is non-zero
             * (typical for VxWorks: 0x00100000; typical for Linux PPC:
             * 0x00000000 -- handled below by leaving load_address alone). */
            if (ih_load != 0) {

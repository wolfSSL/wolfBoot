...
/* Same LPUART0 console as wolfBoot (hal/imx8qm.h). The MMU is off here, so drive the
 * registers directly and leave the baud rate SCFW/ATF set up alone. */
...
/* The DTB pointer arrives as the first argument: el2_to_el1_boot() leaves it in x0 and
 * boot_arm64_start.S forwards it per AAPCS. A parameter stays correct at any -O. */
void __attribute__((section(".boot"))) main(uint64_t dtb) {
    ...
    uart_puthex(dtb);
    uart_puts("\n  DTB magic = ");
    /* Range- and alignment-check before dereferencing: MMU-off this is Device memory,
     * where an unaligned access faults. A stray pointer should be reported, not taken. */

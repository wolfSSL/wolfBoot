/* unit-stm32l4-write.c
 *
 * Regression test for F-12062: the double-word fast path of
 * hal_flash_write() in hal/stm32l4.c was selected on "len - i > 3"
 ...
 *
 * Same harness as the STM32G4 twin (F-11023): extracted functions,
 ...
 * alignment test on the data pointer can pass. The L4 HAL clears the
 * status error bits with a write-1-to-clear store; on the host the
 * clear is a no-op, which models a program that raises no errors.
...
/* Host FLASH register file. SR/CR bit values per RM0394; the L4
 * flash controller has the same status/control layout as the G4. */
typedef struct flash_reg {
    volatile uint32_t SR;
    volatile uint32_t CR;
} flash_reg_t;

static flash_reg_t g_flash_regs;
...
/* W1C clear: no-op on the host, models a program with no errors. */
...
/* The real functions from hal/stm32l4.c (extracted by the Makefile). */
#include "stm32l4_write_extract.h"
...
Suite *stm32l4_write_suite(void)
{
    Suite *s = suite_create("stm32l4-write");
    TCase *tc = tcase_create("stm32l4-write");

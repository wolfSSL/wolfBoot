 ...
 * provides the definitions of these functions. Parameter names match the
 * real MCXA ROM API (fsl_romapi.h): FLASH_EraseSector takes a length in
 * bytes, not a sector count. */
#ifndef FSL_ROMAPI_STUB_H
#define FSL_ROMAPI_STUB_H

status_t FLASH_ProgramPhrase(flash_config_t *config, uint32_t start,
        uint8_t *src, uint32_t length_in_bytes);

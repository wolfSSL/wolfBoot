...
#   CERT_KEY=/path/to/SGK1_1_sha384_secp384r1_v3_usr_crt.pem \
#     tools/scripts/imx8qm/imx8qm-ahab-sign.sh flash.bin [mkimage.log]
#
# The SRK table and keys come from the Code Signing Tool's own PKI scripts;
# this script does not generate them, because the key material must outlive
# any one build. Use keys/ahab_pki_tree.sh, not hab4_pki_tree.sh: the latter
# is the older HAB4 flow for i.MX6/7 and produces keys AHAB will not accept.
# srktool needs --ahab_ver for the table. Note the SGK certificates the AHAB
# script emits are named SGK<n>_1_..., with the extra index.
#
# CERT_KEY is mandatory: ahab_pki_tree.sh sets the CA flag on the SRKs, and a
# CA-flagged SRK may only sign another key, never data, so the container is
# signed by an SGK installed via [Install Certificate]. CST requires a
# Permissions bitmask with it; CERT_PERMISSIONS overrides the 0x1 default.
set -e
...
for v in CST_PATH SRK_TABLE SRK_KEY CERT_KEY; do
    # Indirect expansion, not eval: eval on environment-controlled content
    # would execute a value containing $(...).
    if [ -z "${!v}" ]; then
...
# Not "mktemp -t <template>": GNU deprecates it and BSD/macOS read -t as a
# prefix, not a template, so the same line behaves differently per platform.
CSF="$(mktemp "${TMPDIR:-/tmp}/imx8qm-ahab-XXXXXX.csf")"
...
[Install Certificate]
    File = "$CERT_KEY"
    Permissions = ${CERT_PERMISSIONS:-0x1}
...
# Sign to a separate output. The CSF's "File" stays the unsigned input, so the
# input is never the output: pointing both at one path makes the CST truncate
# it on open and then fail reading it ("Unexpected read termination"). A failed
# run therefore leaves the input untouched, which is the point.
SIGNED="${IMAGE%.bin}-signed.bin"
"$CST" -i "$CSF" -o "$SIGNED"

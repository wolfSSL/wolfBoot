...
# OpenOCD's stmqspi driver addresses XSPI2 NOR via its physical/secure
# alias 0x70000000. Boot and update partition addresses are read from
# .config so this script tracks whatever partition layout the build is
# using (non-TZ: boot 0x70020000; TZ: boot 0x70180000).
BOOT_ADDR=$(grep -E '^WOLFBOOT_PARTITION_BOOT_ADDRESS\?*=' "${WOLFBOOT_ROOT}/.config" 2>/dev/null \
    | head -1 | sed 's/.*=//;s/[[:space:]]//g')
UPDATE_REL=$(grep -E '^WOLFBOOT_PARTITION_UPDATE_ADDRESS\?*=' "${WOLFBOOT_ROOT}/.config" 2>/dev/null \
    | head -1 | sed 's/.*=//;s/[[:space:]]//g')
[ -z "$BOOT_ADDR" ] && BOOT_ADDR=0x70020000
[ -z "$UPDATE_REL" ] && UPDATE_REL=0x00120000
UPDATE_ADDR=$(printf "0x%08x" $(( 0x70000000 + ${UPDATE_REL} )))
...
if [ $SKIP_BUILD -eq 0 ]; then
    ...
    if [ $TEST_UPDATE -eq 1 ]; then
        ...
        HDR_SIZE=$(grep -E '^IMAGE_HEADER_SIZE\?*=' .config | head -1 | sed 's/.*=//;s/[[:space:]]//g')
        [ -z "$HDR_SIZE" ] && HDR_SIZE=256
        [ -f tools/keytools/sign ] || make -C tools/keytools
        # sign tool reads IMAGE_HEADER_SIZE from env; must match wolfBoot build
...
if [ $APP_ONLY -eq 0 ]; then
    ...
    if [ -n "$SIGN_TOOL" ]; then
        ...
        # Match ST's reference template invocation (UM3234, Template_FSBL_XIP).
        # Do NOT pass -la/-ep: the tool extracts the entry point (with the
        # Thumb bit) from the Cortex-M vector table at offset 0x04 of the
        # binary. Passing -ep <addr> writes <addr> verbatim into the header
        # entry-point field with no Thumb bit, so the Boot ROM jumps to an
        # ARM-mode instruction on a Cortex-M55 and instant hard faults.
        if ! "$SIGN_TOOL" -bin "${WOLFBOOT_ROOT}/wolfboot.bin" -nk -of 0x80000000 -t fsbl -hv 2.3 -align -o "${TRUSTED_BIN}"; then
            echo -e "${RED}Error: STM32_SigningTool_CLI failed${NC}"; exit 1
        fi
        [ -s "${TRUSTED_BIN}" ] || { echo -e "${RED}Failed to generate trusted binary${NC}"; exit 1; }
        # Sanity-check magic ("STM\x32" little-endian = 0x324d5453 at offset 0)
        MAGIC=$(od -A n -t x4 -N 4 "${TRUSTED_BIN}" | awk '{print $1}')
        if [ "$MAGIC" != "324d5453" ]; then
            echo -e "${RED}Error: bad FSBL header magic (${MAGIC})${NC}"; exit 1
        fi

...
## TPM based keystore (public keys)
ifeq ($(WOLFBOOT_TPM_KEYSTORE),1)
  ifneq ($(WOLFBOOT_TPM_KEYSTORE_NV_INDEX),)
    ifneq ($(WOLFBOOT_TPM_POLICY_NV_INDEX),)
      WOLFTPM:=1
      CFLAGS+=-DWOLFTPM_KEYSTORE
      CFLAGS+=-DWOLFTPM_KEYSTORE_INDEX=$(WOLFBOOT_TPM_KEYSTORE_NV_INDEX)
      CFLAGS+=-DWOLFTPM_POLICY_DIGEST_INDEX=$(WOLFBOOT_TPM_POLICY_NV_INDEX)

      # Make sure AES CFB is enabled in wolfCrypt for parameter encryption
      CFLAGS+=-DWOLFSSL_AES_CFB

      ifeq ($(WOLFBOOT_TPM_ENCRYPT_KEYSTORE),1)
        ifneq ($(WOLFBOOT_TPM_ENCRYPT_KEYSTORE_NV_INDEX),)
          CFLAGS+=-DWOLFTPM_ENCRYPT_KEYSTORE
          CFLAGS+=-DWOLFTPM_ENCRYPT_KEYSTORE_INDEX=$(WOLFBOOT_TPM_ENCRYPT_KEYSTORE_NV_INDEX)
        endif
      endif

      ifeq ($(WOLFBOOT_TPM_PCR_INDEX),)
        # The lower PCR's should be used for production

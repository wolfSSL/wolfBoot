...
/*** FLASH ***/
#define APB1_CLOCK_ER           (*(volatile uint32_t *)(RCC_BASE + 0x1C)) //I think?
#define APB1_CLOCK_RST          (*(volatile uint32_t *)(RCC_BASE + 0x10))
...
void RAMFUNCTION flash_set_waitstates(int waitstates)
{
    // FLASH_ACR |=  waitstates | FLASH_ACR_ENABLE_DATA_CACHE | FLASH_ACR_ENABLE_INST_CACHE;
}

RAMFUNCTION void flash_wait_complete(void)
{
...
void RAMFUNCTION clear_errors(void)
{
...
int RAMFUNCTION hal_flash_write(uint32_t address, const uint8_t *data, int len)
{
    ...
    // FLASH_CR &= (~(0x03 << 8));
    if (address & 0x01) {
        //flash writes need 16-bit alignment
...
void RAMFUNCTION flash_erase_page(uint32_t page)
{
    ...
    FLASH_CR &= ~FLASH_CR_PER;
    // TODO: check EOP flag in flash SR
    // TODO: clear EOP flag
}

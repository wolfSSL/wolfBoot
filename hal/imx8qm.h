...
/* NXP i.MX 8QuadMax (MIMX8QM) definitions for the MCIMX8QM-MEK; register bases from
 * the DTB. boot_aarch64_start.S includes this, so keep assembly off the C-only pads. */
...
/* BL33 image layout. ATF enters at 0x80020000, where u-boot.bin links, so the stock
 * imx-mkimage recipe fits. Keep in sync with imx8qm.ld and imx8qm-mkflashbin.sh. */
...
/* Update partition, staged in low DRAM above the BL33 image. A constant, not a linker
 * symbol: an ADRP against an absolute symbol this far out exceeds its +/-4GB reach. */
#define IMX8QM_UPDATE_ADDR          0x88000000

/* Generic timer fallback if firmware left CNTFRQ_EL0 unprogrammed; the i.MX8QM system
 * counter runs at 8 MHz. */
#ifndef TIMER_CLK_FREQ
#define TIMER_CLK_FREQ              8000000
#endif

/* Console LPUART0: stdout-path in imx8qm-mek.dts, J11 FTDI interface 0, 115200 8N1.
 * 32-bit little-endian map (UPIO_MEM32) with reg_off 0x10, folded into the offsets. */
...
/* uSDHC: usdhc1 is the 8-bit eMMC, usdhc2 the 4-bit SD socket (card detect on
...
/* SYS_CTRL. Reset and data-timeout sit at the same bits as SDHCI's SRS11 and pass
 * through the shim; the DVS/SDCLKFS clock divider does not and is programmed directly. */
...
/* MIX_CTRL, the transfer-mode half. Bits 0,1,2,4,5 match standard SDHCI; the tuning
 * and DDR bits above them have no equivalent and must be preserved. */
...
/* WTMK_LVL burst watermarks, in 4-byte words. A full 512-byte block so the PIO loop
 * sees one Buffer Read Ready per block rather than one per 32 bytes. */
...
/* uSDHC per clock, owned by the SCU; 198 MHz is what the NXP BSP programs. With
 * IMX8QM_SCU=1 the real rate is read back and this is only the fallback. */
#ifndef IMX8QM_USDHC_PERCLK_HZ
#define IMX8QM_USDHC_PERCLK_HZ      198000000
#endif

/* FlexSPI0: MT35XU512ABA 64 MB octal NOR. AHB read window plus the LUT-driven IP
 * command path for erase/program; same IP as the Layerscape XSPI (nxp_ls1028a.c). */
...
/* The part also documents a 4 KB subsector erase (0x21), but 128 KB is advertised and
 * is what U-Boot and Linux use. Being wrong the unsafe way is unrecoverable. */
#define FLEXSPI_NOR_SECTOR_SIZE     0x20000
#define FLEXSPI_NOR_PAGE_SIZE       0x100    /* confirmed by "sf probe" */

/* Messaging Unit: A-core software reaches the SCU over MU1_A, the channel free for
 * BL33 (TF-A uses MU0). Only compiled with IMX8QM_SCU=1. */
...
#define SC_R_SMMU                   17
/* SMMU stream ID both uSDHC controllers present, from "iommus" on mmc@5b010000 and
 * mmc@5b020000 (<&smmu 0x11 0x7f80>). Identical, hence one Linux iommu group. */
...
/* uSDHC pads from the MEK pin groups, all mux function 0; note USDHC1_DATA2 is 227,
 * not 226. The pad ring names the SD slot USDHC1_* but it is uSDHC2: follow the base. */
...
/* Low Power Clock Gating. sc_pm_clock_enable() ungates at the SCU; the peripheral's
 * own LPCG cell still needs opening, with the double-write the reference driver uses. */

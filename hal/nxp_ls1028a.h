...
/* TZC-400 Region 0 ID-access register (non-secure permissions). Region
 * attributes (0x110) only gate SECURE access; non-secure access is controlled
 * here. bits[15:0]=per-NSAID read enable, bits[31:16]=per-NSAID write enable.
 * 0xFFFFFFFF permits non-secure read+write for all NSAIDs -- needed so the
 * non-secure ENETC bus-master DMA can reach DDR. */

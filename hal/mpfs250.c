...
static int setup_phy(void)
{
    ...
    /* HSS captured value at this PHY offset on a working board today
     * (2026-05-15 HSS DEBUG HEXDUMP from /tmp/hss-phy-regs-2026-05-15.txt):
     *   PHY 0x770 (rpc220) = 0x0C
     *   PHY 0x788 (rpc226) = 0x14
     * Previously wrote 0x01 here believing that was the HSS value --
     * actually 0x01 was a transient state, the operational post-train
     * value is 0x0C.  Writing the wrong value here was contributing to
     * the AXI read stall. */
    DDRPHY_REG(PHY_RPC220) = 0x0000000CUL;  /* HSS-captured operational value */
 ...
 * 5524-5546) which rotates the picked min_refclk/min_diffp1 across
 * retries to converge when the first pick is marginal. */
static int run_training(uint32_t retry_count)
{
    ...
    DBG_DDR("    MR writes...");
    {
        ...
        /* MR2 = 0x2D: bits[5:3]=101=nWR option 5, bits[2:0]=101=
         * RL/WL Set 5 -> RL=28/WL=14.  Tested 0x12 (RL=14 matching
         * CFG_RL=14) -- training still completes but reads still
         * hang.  Reverted to 0x2D as training-stable baseline.
         * The CL/RL mismatch hypothesis was wrong; reads hang for
         * a deeper reason. */
    ...
    DBG_DDR("    ADDCMD training...");
    {
        ...
        while (a5_offset_status != 0 && max_retries > 0) {
            ...
            uint32_t min_diffp2 = 0xFF;
            uint32_t min_refclk = 0x8;
            uint32_t min_refclkp1 = 0x8;
            ...
            for (j = 0; j < 8; j++) {
                if (difference[j] < min_diff) {
                    min_refclk = j;
                    min_refclkp1 = (j + 1) & 0x7;
                    min_diff = difference[j];
                    min_diffp1 = difference[(j + 1) & 0x7];
                    min_diffp2 = difference[(j + 2) & 0x7];
                }
            }

            if (min_diff == 0xFF)
                a5_offset_status = 1;
            /* HSS check: if no valid refclk index found, flag low-freq
             * fail.  Out-of-range sentinel (8) means no transition. */
            if (min_refclk == 0x8U)
                a5_offset_status = 1;

            /* MOVE_CK retry rotation (HSS mss_ddr.c:5524-5546).  HSS
             * always defines MOVE_CK and cycles through 3 different
             * (min_refclk, min_diffp1) pairs across retries so that
             * if the first pick is marginal, alternative phase/dly
             * combinations are tried.  LPDDR4 Video Kit Libero values:
             *   0 deg : retry%3 == 0 -- use min_diff, min_refclk-1
             *   45 deg: retry%3 == 1 -- no change
             *   90 deg: retry%3 == 2 -- use min_diffp2, min_refclkp1
             * Only applied when ADDCMD passed (a5_offset_status==0). */
            if (a5_offset_status == 0) {
                uint32_t move = retry_count % 3U;
                if (move == 0U) {
                    min_diffp1 = min_diff;
                    min_refclk = (min_refclk - 1U) & 0x7U;
                } else if (move == 2U) {
                    min_diffp1 = min_diffp2;
                    min_refclk = min_refclkp1;
                }
                /* move == 1 (45 deg): leave as-is */
            }

            DBG_DDR(" a5_max=%d retry=%d move=%d min_refclk=%d min_diff=%d min_diffp1=%d min_diffp2=%d status=%d ",
                transition_a5_max, retry_count, retry_count % 3U,
                min_refclk, min_diff, min_diffp1, min_diffp2,
                a5_offset_status);

            if (a5_offset_status == 0) {
                /* HSS-equivalent refclk_offset addition (mss_ddr.c:5549).
                 * For Video Kit LPDDR4 1600 Mbps, OFFSET_0 = 3.
                 * Final phase = (refclk_offset + min_refclk) & 0x7.
                 *
                 * 2026-05-11: tried hardcoding min_refclk=1 to match
                 * HSS's choice on this board -- result was train_stat
                 * stuck at 0x1 across all retries, worse than our
                 * sweep result.  Our pre-ADDCMD PHY state must differ
                 * from HSS's, so the same final phase value isn't
                 * universally good -- we need our own sweep, but it
                 * needs to be more deterministic. */
...
int mpfs_ddr_init(unsigned int outer_retry)
{
     ...
     */
    {
         ...
         * cycles through all three alternative (min_refclk, min_diffp1)
         * picks in one DDRC-init pass.  Combined with the 6 outer
        ...
        while (train_retry < MAX_TRAIN_RETRY) {
            ...
            /* decrement_dq: shift per-lane DQ output delays down via
             * expert mode pulses.  Our per-lane wl_dly is 0x24-0x2C
             * (36-44); HSS post-train is 0x17-0x1A (23-26) -- our
             * delays are ~13 taps higher than HSS-on-board.  TIP
             * trained wl_dly autonomously; can't override directly
             * (read-only register).  Pulsing PHY_EXPERT_DLYCNT_MOVE0
             * with DIRECTION0=0 shifts the delay-line down N counts
             * per lane (3 pulses per tap movement, HSS's increment_dq
             * convention at mss_ddr.c:2976).
             *
             * Empirically this is REQUIRED for AXI reads to complete
             * on this board after the 38-register offset bugfix.
             * Without it, naked reads hang on every boot even with
             * train_stat=0x1F and ADDCMD dly=7.  The 26-tap shift
             * brings effective wl_dly close to HSS's 23-26 range. */
            {
                const uint32_t target_count = 26U;
                const uint32_t decrement_count = target_count * 3U;
                uint32_t k;
                wolfBoot_printf("  decrement DQ per-lane by %u counts...\n",
                    (unsigned)target_count);
                for (lane = 0; lane < 4U; lane++) {
                    DDRPHY_REG(PHY_EXPERT_DLYCNT_MOVE0) = 0x00UL;
                    DDRPHY_REG(PHY_EXPERT_DLYCNT_DIRECTION0) = 0x00UL;
                    DDRPHY_REG(PHY_EXPERT_DFI_STATUS_TO_SHIM) = 0x07UL;
                    DDRPHY_REG(PHY_EXPERT_MODE_EN) = 0x21UL;
                    mb();
                    for (k = 0; k < decrement_count; k++) {
                        DDRPHY_REG(PHY_EXPERT_DLYCNT_MOVE0) =
                            (0xFFUL << (lane * 8UL));
                        DDRPHY_REG(PHY_EXPERT_DLYCNT_MOVE0) = 0x00UL;
                    }
                    DDRPHY_REG(PHY_EXPERT_MODE_EN) = 0x08UL;
                ...
                wolfBoot_printf("  decrement DQ done\n");
            }
            train_stat = DDRPHY_REG(PHY_TRAINING_STATUS);
            wolfBoot_printf(
    ...
    /* Step 10: Memory test (diagnostic only -- non-fatal).
     *
     * memory_test() uses CPU cached 4-byte writes through L2 +
     * L2_FLUSH64 to force write-back to DDR.  Lane 2/3 of the L2
     * burst-write path is broken: lower bytes (lanes 0/1) reach DDR
     * but upper bytes don't.  PDMA writes (which the real disk-load
     * uses via SDHCI) write 64-byte bursts directly through the AXI
     * port and DO reach all 4 lanes -- verified by mpfs_clear_bootup
     * _cache_ways completing cleanly above, and by HSS's MEMTEST CLI
     * command passing on this board.
     *
     * So memory_test failure here doesn't predict disk-load failure.
     * Print the result and proceed -- the disk-load path uses the
     * working PDMA write path. */
    ret = memory_test();
    if (ret != 0) {
        wolfBoot_printf("DDR: memory_test FAILED (cached path) -- "
            "proceeding to disk-load (uses PDMA, expected to work)\n");
    }
    ret = 0;

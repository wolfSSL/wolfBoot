 ...
 *
 * Two modes supported (selected at build time):
 * - MPFS_SC_SPI=1: System Controller SPI services (read-only)
 * - MPFS_SC_SPI=0: MSS QSPI Controller direct access (read/write/erase)
 * ========================================================================== */

#ifndef MPFS_SC_SPI
/* ==========================================================================
 * MSS QSPI Controller Functions (direct peripheral access)
 * ========================================================================== */
...
static void qspi_flash_wakeup(void)
{
    ...
    /* Flash needs tRES1 (3us typ) to wake up - add small delay */
 ...
 *
 * PolarFire SoC supports two SPI flash access methods:
 *
 * MPFS_SC_SPI=1 (default): System Controller SPI services
 *   - Uses SCB mailbox (MSS_SYS_spi_copy) for fabric-connected flash
 *   - This is how HSS loads images from design flash
 *
 * MPFS_SC_SPI=0: MSS QSPI Controller direct access
 *   - Uses QSPI peripheral at 0x21000000
 *   - For external flash connected to MSS QSPI pins
 */
void qspi_init(void)
{
#ifdef MPFS_SC_SPI
    wolfBoot_printf("QSPI: Using System Controller SPI services\n");
#else
    uint8_t id[3];
                        ...
                        uint8_t *data, uint32_t data_len, uint8_t dummy_cycles)
{
     ...
     * - Command bytes: number of TX-only bytes before data phase
     * - Idle cycles: inserted by hardware between command and data
     * - FWORD: set for word-aligned transfers (per Microchip reference)
     */
    frames = ((total_bytes & 0xFFFF) << QSPI_FRAMES_TOTALBYTES_OFFSET) |
             ((cmd_len & 0x1FF) << QSPI_FRAMES_CMDBYTES_OFFSET) |
             ((dummy_cycles & 0xF) << QSPI_FRAMES_IDLE_OFFSET) |
             (1u << QSPI_FRAMES_FWORD_OFFSET);
    ...
    for (i = 0; i < cmd_len; i++) {
        timeout = 100000;
        while ((QSPI_STATUS & QSPI_STATUS_TXFULL) && --timeout);
        if (timeout == 0) {
...
#ifdef DEBUG_QSPI
    wolfBoot_printf("QSPI: Cmd sent, status=0x%x, read_mode=%d\n",
        QSPI_STATUS, read_mode);
#endif

    if (read_mode) {
        /* Wait for command TX to complete before reading response */
        timeout = 100000;
        while (!(QSPI_STATUS & QSPI_STATUS_TXDONE) && --timeout);
        if (timeout == 0) {
            wolfBoot_printf("QSPI: TX done timeout (read), status=0x%x\n",
                QSPI_STATUS);
            return -5;
        }

        /* Read mode: receive data after command + idle cycles */
...
#ifdef MPFS_SC_SPI
/* ==========================================================================
 * System Controller SPI Flash Services
 *
 * The SPI flash on PolarFire SoC is accessed through System Controller
 * services via the SCB mailbox, NOT through direct QSPI peripheral access.
 * This matches how HSS accesses the flash using MSS_SYS_spi_copy().
 * ========================================================================== */

/**
 * sc_spi_copy - Copy data from SPI flash to memory using System Controller
 *
 * @dest_addr: Destination address in MSS memory (where data will be copied)
 * @flash_addr: Source address in SPI flash
...
static int sc_spi_copy(uint64_t dest_addr, uint32_t flash_addr, uint32_t len)
{
     ...
     * Bytes 12-15: byte count (32-bit)
     * Byte 16:     options (clock speed: 1=40MHz, 2=20MHz, 3=13.33MHz)
     */
    mb_data[0]  = (dest_addr >>  0) & 0xFF;
    mb_data[1]  = (dest_addr >>  8) & 0xFF;
    mb_data[2]  = (dest_addr >> 16) & 0xFF;
    mb_data[3]  = (dest_addr >> 24) & 0xFF;
    mb_data[4]  = (dest_addr >> 32) & 0xFF;
    mb_data[5]  = (dest_addr >> 40) & 0xFF;
    mb_data[6]  = (dest_addr >> 48) & 0xFF;
    mb_data[7]  = (dest_addr >> 56) & 0xFF;
    mb_data[8]  = (flash_addr >>  0) & 0xFF;
    mb_data[9]  = (flash_addr >>  8) & 0xFF;
    mb_data[10] = (flash_addr >> 16) & 0xFF;
    mb_data[11] = (flash_addr >> 24) & 0xFF;
    mb_data[12] = (len >>  0) & 0xFF;
    mb_data[13] = (len >>  8) & 0xFF;
...
{
#ifdef MPFS_SC_SPI
    /* System Controller SPI does not support write operations */
    (void)address;
    (void)data;
    (void)len;
    wolfBoot_printf("SC SPI: Write not supported\n");
    return -1;
#else
...
{
#ifdef MPFS_SC_SPI
    /* Use System Controller SPI copy service (like HSS does) */
    int ret;
    #ifdef DEBUG_QSPI
    wolfBoot_printf("SC SPI: Read 0x%x -> 0x%lx, len %d\n",
        (uint32_t)address, (unsigned long)data, len);
    #endif
    ret = sc_spi_copy((uint64_t)(uintptr_t)data, (uint32_t)address, (uint32_t)len);
    /* Return length on success (expected by wolfBoot), negative on error */
    return (ret == 0) ? len : ret;
#else
    /* Use MSS QSPI Controller direct access */
    #ifdef DEBUG_QSPI
...
{
#ifdef MPFS_SC_SPI
    /* System Controller SPI does not support erase operations */
    (void)address;
    (void)len;
    wolfBoot_printf("SC SPI: Erase not supported\n");
    return -1;
#else
